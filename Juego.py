...
class Juego:
    ...
    def leerArchivoPreguntas (self):
        archivo = open("archivos/Preguntas.csv","r",encoding='utf-8')
    ...
    def leerTablero (self):
        archivo = open("archivos/Tablero.csv","r",encoding='utf-8')
        self.tablero = Lista()

        linea = archivo.readline()
        while (linea != ""):
            datos = linea.split(";")
            casilla = Casilla(int(datos[0]), int(datos[1]), int(datos[2]), int(datos[3]), int(datos[4][:-1]))
            self.tablero.agregarNodo(casilla)
            linea = archivo.readline()
        archivo.close()

    def leerArchivoPreguntasRuleta (self):
        archivo = open("archivos/Preguntas-VoF.csv","r",encoding='utf-8')
    ...
    def aumentarTurno (self):
        self.turno += 1
        self.turno = self.turno % self.num_jug

    def jugar (self):
        self.turno = 0
        while (self.verificarGanador() < 0):
            print("Turno del jugador ", self.turno)
            print("-> Tirando los dados")
            dado = self.tirarDado()
            print("\t-> Resultado del dado: ", dado)

            if (self.jugadores[self.turno] + dado > 30):
                print("\t-> Pierdes tu turno!! Tienes que llegar exactamente a la casilla 30 para ganar")
            else:
                self.jugadores[self.turno] += dado
                self.casillaCaida(self.jugadores[self.turno])
